...
class VerResultados extends SimpleWindow<AgenciaDeLoteria>{
	...
	override protected addActions(Panel actionsPanel) {
		// a la lista no le agregamos ninguna accion
	}
	...
	override protected createFormPanel(Panel mainPanel) {
		...
		new Table<ResultadoLoteria>(mainPanel, ResultadoLoteria) => [
			...
			height = 1500           // <== no tiene efecto, se arma en base al tamaño de las columnas con fixedSize(int)
			numberVisibleRows = 5   // <== termina definiendo el alto
			...
			new Column<ResultadoLoteria>(it) => [
				...
				fixedSize = 250   //el tamaño que va a tener
